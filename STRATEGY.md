...
3. If `TARGET_COMP_MIN`/`MAX` env vars are configured, compare the visitor's figure against that band; if below band, respond along the lines of: *"That's likely below where Eduardo's experience level sits — he may be a more senior profile than this role needs... Do not soften this into stating a number instead."* If the band isn't configured, the prompt forbids judging any figure at all — silence on the band, not a guess.

`FACT:` (fixed 2026-09-10) that last sentence described the intent, not the behavior. Until this date the aligned/below-band comparison bullets were **static text outside the `hasBand` ternary**, so they shipped even with no band configured — the same numbered item told the model both "you have no number to compare against" and "if the figure is below the band, say he may be a more senior profile than this role needs." A contradiction, and the scripted branch carried verbatim wording, which is the stronger attractor. The bullets now live inside the configured-band branch, and the no-band clause additionally forbids characterizing Eduardo as too senior, too junior, or too expensive. `DEPLOY-GUIDE.md` recorded the same incorrect belief and was corrected in the same pass.
