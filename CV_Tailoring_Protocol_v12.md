# CV Tailoring Protocol — Eduardo Castro (v12)
...
### Register re-check (runs at end of Step 3, before handoff to Step 5)

After the section walk is complete, read the following lines only — no other bullets:

- The summary opening (first sentence)
- The first responsibility bullet of each role
- The first wins bullet of each role

For each, ask: is the sentence subject a decision or accountability, or did it drift back to an activity during the keyword walk?

**Drift signal:** sentence opens with activity verbs (managed, executed, designed, achieved, delivered, launched) where a decision verb was present after Step 0's rewrite pass, or where the keyword pressure introduced one.

**Fix:** apply Rewrite rule 1 (responsibility bullets and summary) or Rewrite rule 2 (wins bullets) to restore the decision subject. The fix is a subject-level correction only — do not alter the keyword content, metric, or length of the bullet beyond what the subject change requires.

**Constraint:** the register re-check is not a keyword placement opportunity. No new keywords enter during this pass. If correcting the subject drops a keyword that was load-bearing in that bullet, move the keyword to the next available bullet in the same subsection before exiting Step 3. Do not re-insert the displaced keyword into the corrected bullet.

This pass runs silently. Nothing surfaces in the chat unless a judgment call is required.
...
- Register re-check is mandatory at the end of Step 3. Keyword pressure silently degrades sentence subjects from decisions back to activities; this pass catches and fixes it before Step 5.
